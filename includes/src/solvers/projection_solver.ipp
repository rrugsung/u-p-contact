...
void mpm::ProjectionSolver::solve_pressure_poisson_equation(double dt) {
  ...
  //std::cout << "K_L2_ \n" << K_L2_ << "\n\n";
  //std::cout << "K_L3_ \n" << K_L3_ << "\n\n";
  //std::cout << "intermediate_solid_velocity \n" << intermediate_solid_velocity_ << "\n\n";
  //std::cout << "intermediate_water_velocity \n" << intermediate_water_velocity_ << "\n\n";
...
void mpm::ProjectionSolver::element_mapping_matrix() {
  ...
  element_map_.resize(num_elem, gimpNodes);
...
void mpm::ProjectionSolver::build_L_matrix() {
  unsigned elem = 0;
  Eigen::Matrix<double,gimpNodes,gimpNodes> elem_L;
  for (const auto &elem_ptr : element_ptrs_) {
    Eigen::Matrix<mpm::Node*, 1, gimpNodes> elem_nodes = elem_ptr->give_element_gimp_node_ptrs();
    elem_L = elem_ptr->give_L_matrix();
    //std::cout << "elem_L: \n" << elem_L << "\n";
    for (unsigned i = 0; i < gimpNodes; i++) {
      auto nphase1 = elem_nodes(i)->give_node_phase();
      if (nphase1 == 2) {
        for (unsigned j = 0; j < gimpNodes; j++) {
          auto nphase2 = elem_nodes(j)->give_node_phase();
          if (nphase2 == 2) {
...
void mpm::ProjectionSolver::build_K_L_matrix() {
  unsigned elem = 0;
  Eigen::Matrix<double,gimpNodes,dim*gimpNodes> elem_K_L2, elem_K_L3;
  for (const auto &elem_ptr : element_ptrs_) {
    Eigen::Matrix<mpm::Node*, 1, gimpNodes> elem_nodes = elem_ptr->give_element_gimp_node_ptrs();
        ...
        for (unsigned j = 0; j < gimpNodes; j++) {
          ...
          if (nphase2 == 2) {
            for (unsigned k = 0; k < dim; k++) {
              unsigned row = element_map_(elem, i);
              unsigned col = element_map_(elem, j) + (num_nodes_ * k);
              K_L2_.coeffRef(row,col) += elem_K_L2(i,(j + k*gimpNodes));
              K_L3_.coeffRef(row,col) += elem_K_L3(i,(j + k*gimpNodes));
            }
          }
        }
      }
...
void mpm::ProjectionSolver::build_K_S2_matrix() {
  unsigned elem = 0;
  Eigen::Matrix<double,dim*gimpNodes,gimpNodes> elem_K_S2;
  for (const auto &elem_ptr : element_ptrs_) {
    Eigen::Matrix<mpm::Node*, 1, gimpNodes> elem_nodes = elem_ptr->give_element_gimp_node_ptrs();
        ...
        for (unsigned j = 0; j < gimpNodes; j++) {
          ...
          if (nphase2 == 2) {
            for (unsigned k = 0; k < dim; k++) {
              unsigned row = element_map_(elem, i) + (num_nodes_ * k);
              unsigned col = element_map_(elem, j);
              K_S2_.coeffRef(row,col) += elem_K_S2((i + k*gimpNodes),j);
            }
          }
        }
      }
