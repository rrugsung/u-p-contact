...
public:
  ...
  void append_node_normal_distance(unsigned id, double normal_distance, double tangential_distance) {
    if (id == 0) {
      auto t = std::make_tuple(normal_distance,tangential_distance);
      distances0_.push_back(t);
    }
    else if(id == 1) {
      auto t = std::make_tuple(normal_distance,tangential_distance);
      distances1_.push_back(t);
    }
  }
  ...
  // print nodal phase
  void print_material_ids() {
    std::cout << "nid: " << nid_ << "mat_ids: " << material_ids_.size() << "\n";
  }
  ...
  void compute_multimaterial_normal_unit_vectors(double coord);
  ...
  void assign_multimaterial_nsolid_initial_velocities(const unsigned mat_id, const Eigen::Matrix<double,1,dim>& velocity){
    nsolid_initial_velocities_(mat_id,1) = velocity(1);
  }

  void assign_nsolid_initial_velocity(const Eigen::Matrix<double,1,dim>& velocity){
    nsolid_initial_velocity_(1) = velocity(1);
  }  
  ...
  void assign_nodal_multimaterial_pressure_increments(const unsigned mat_id, const double &incrementXmass) {
    npressure_increments_(mat_id) += incrementXmass;
  }
  ...
  void compute_nodal_initial_velocity();
  
  // adjust initial multimaterial velocity to satisfy impenetrability condition
  void adjust_nodal_initial_velocity();

  // compute nodal pressure from the mapped values : OK
  void compute_nodal_pressure();

  void compute_nodal_pressure_increment();
  ...
  bool give_node_contact_status() {
    return contact_status_;
  }

  std::set<unsigned> give_node_material_ids() const{
  ...
  //!
  std::vector<std::tuple<double, double> > distances0_;
  std::vector<std::tuple<double, double> > distances1_;
private:
