...
protected:                    
  ...
  static const unsigned gimpNodes = mpm::constants::GIMPNODES;
...
public:
  ...
  //! param[in] node_id id of the node
  //! param[in] node_ptr pointer to the node
  void set_element_gimp_nodes(const unsigned& i, const unsigned& node_id, mpm::Node* node_ptr) {
    gimp_elem_nodes_id_(i) = node_id;
    gimp_elem_nodes_ptrs_(i) = node_ptr;
  }

  void check_gimp_nodes(){
    if(elem_id_ == 13) {
      std::cout << "ids: \n" << gimp_elem_nodes_id_ << "\n";
      for (unsigned i=0; i<gimpNodes; i++){
        unsigned nid = gimp_elem_nodes_ptrs_(i)->give_id();
        std::cout << "ptr_ids: \n" << nid << "\n";
      }
    }
  }

  // set the uring elements
  ...
  void add_to_element_particle_density(const double& p_volume, const double& tp_p_volume, const double& p_density) {
    elem_particle_density_ += 1;
    elem_material_volume_ += p_volume;
    elem_tp_material_volume_ += tp_p_volume;
  ...
  void append_particle_coordinate(const Eigen::Matrix<double,1,dim> coord){
    p_coords_.push_back(coord);
  }
  ...
  // give the vector of pointers to the gimp nodes of the element
  //! param[out] vector of pointers to element gimp nodes
  Eigen::Matrix<mpm::Node*, 1, gimpNodes> give_element_gimp_node_ptrs() const {
    return gimp_elem_nodes_ptrs_;
  }
  ...
  // give material volume / element volume ratio
  //! param[out] ratio volume of particles in element / element volume
  double give_ratio_tp_particle_volume_to_element_volume() const {
    double ratio = elem_tp_material_volume_ / elem_volume_;
    return ratio;
  }
  ...
  std::set<unsigned> give_element_material_ids () {
    return material_ids_;
  }

  std::vector<Eigen::Matrix<double,1,dim>> give_element_particle_coordinates() {
    return p_coords_;
  }
  ...
  Eigen::Matrix<double,gimpNodes,gimpNodes> give_L_matrix() const {
    //if (std::fabs(elem_material_volume_ / elem_volume_) < beta)
      return L_mp_;
      // else
      // return L_gp_;
  ...
  Eigen::Matrix<double,gimpNodes,dim*gimpNodes> give_K_L2_matrix() const {
    //if (std::fabs(elem_material_volume_ / elem_volume_) < beta)
    return K_L2_mp_;
    // else
    //   return K_L2_gp_;
  ...
  Eigen::Matrix<double,gimpNodes,dim*gimpNodes> give_K_L3_matrix() const {
    //if (std::fabs(elem_material_volume_ / elem_volume_) < beta)
      return K_L3_mp_;
      // else
      // return K_L3_gp_;
  ...
  // give K_S2 matrix
  //! param[out] K_S2_
  Eigen::Matrix<double,dim*gimpNodes,gimpNodes> give_K_S2_matrix() const {
    //if (std::fabs(elem_material_volume_ / elem_volume_) < beta)
      return K_S2_mp_;
      // else
      // return K_S2_gp_;
...
protected:
  ...
  //! GIMP element nodes.
  Eigen::Matrix<unsigned, 1, gimpNodes> gimp_elem_nodes_id_;
  Eigen::Matrix<mpm::Node*, 1, gimpNodes> gimp_elem_nodes_ptrs_;

  //! Material ids whose information was passed to this element
  std::set<unsigned> material_ids_;

  //! Two-phase particle coordinates
  std::vector<Eigen::Matrix<double,1,dim>> p_coords_;
  ...
  double elem_tp_material_volume_;
  ...
  Eigen::Matrix<double, gimpNodes, gimpNodes> L_mp_;
  Eigen::Matrix<double, gimpNodes, dim*gimpNodes> K_L2_mp_;
  Eigen::Matrix<double, gimpNodes, dim*gimpNodes> K_L3_mp_;
  Eigen::Matrix<double, dim*gimpNodes, gimpNodes> K_S2_mp_;
  // ******************************************************
