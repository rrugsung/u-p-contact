...
public: 
  ...
  void set_element_and_nodes(mpm::Element* &ptr_e, Eigen::Matrix<mpm::Node* ,1, numNodes> &ptrs_n, Eigen::Matrix<mpm::Node*,1,gimpNodes> &ptrs_gn) {
        element_ = ptr_e;
        nodes_   = ptrs_n;
        gimp_nodes_ = ptrs_gn;
  }
  ...
  void assign_velocity();

  void append_normal_distance_to_nodes();
...
//####################################
  ...
  //! Assign material id of this particle to nodes and an element
  void append_material_id();

  //! Assign two-phase particle coordinate of this particle to an element
  void append_particle_coordinate_to_element();
  ...
  void map_pressure_increment_to_nodes();

  // map pressure from nodes
  void map_pore_pressure_from_nodes();
  void map_pressure_increment_from_nodes();
...
//#####################################
  ...
  void check_preconsolidation_pressure();
...
//#####################################
  ...
  void compute_total_solid_stress();
...
//###########################################
  ...
  Eigen::Matrix<double,1,dim> give_displacement() {
  ...
  // give particle stress
  //!
  Eigen::Matrix<double,1,6> give_stress() const {
    return solid_stress_;
  }

  double give_pressure() const {
    return pore_pressure_;
  }
...
//###############################################
  ...
  // WRITE STRESS TO FILE
  void write_total_stress(std::ostream& oFile);
...
protected:
  ...
  Eigen::Matrix<double, 1, dim> solid_int_rigid_velocity_;
  Eigen::Matrix<double, 1, dim> solid_final_rigid_velocity_;
  Eigen::Matrix<double, 1, dim> solid_rigid_acceleration_;
  ...
  Eigen::Matrix<double, 1, 6> total_solid_stress_; // total stress
  ...
  Eigen::Matrix<double, 1, gimpNodes> shape_fun_;
  Eigen::Matrix<double, 1, gimpNodes> shape_fun_centre_;
  Eigen::Matrix<double, dim, gimpNodes> grad_shape_fun_;
  Eigen::Matrix<double, dim, gimpNodes> grad_shape_fun_centre_;

  std::array<Eigen::Matrix<double, dof, dim>, gimpNodes> B_;
  std::array<Eigen::Matrix<double, dof, dim>, gimpNodes> BBar_;
  std::array<Eigen::Matrix<double, dof, dim>, gimpNodes> BCentre_;
