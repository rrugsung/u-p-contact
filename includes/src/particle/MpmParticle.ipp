...
double mpm::MpmParticle::give_center_coords() {
    double coord=0;
    for (auto& particle : sp_particles_){
        Eigen::Matrix<double,1,dim> coords = particle->give_coordinates();
        coord = coord + coords(1);
    }
    coord = coord / sp_particles_.size();
    return coord;
}
...
void mpm::MpmParticle::write_particle_totalstress_data_to_file(std::ostream& outFile) {
    unsigned numOfParticles = particles_.size();
    ...
    outFile << "POINT_DATA " << numOfParticles << "\n";
    outFile << "VECTORS totalStress float" << "\n";
    for (auto i : particles_)
        i -> write_total_stress(outFile);
...
void mpm::MpmParticle::write_particle_ts_data_to_file(std::ostream& outFile) {
...
void mpm::MpmParticle::write_particle_oed_data_to_file(std::ostream& outFile) {
    unsigned pid;
    Eigen::Matrix<double,1,dim> pCord;
    Eigen::Matrix<double,1,6> pStress;
    Eigen::Matrix<double,1,dim> pDispl;
    double pPressure;
    for (auto i : particles_) {
        pid = i->give_id();
        if (pid == 3) {
            pPressure = i->give_pressure();
            pPressure = pPressure/1000;
            pStress = i->give_stress();
            pStress = pStress/1000;
            pDispl = i->give_displacement();
            outFile << pPressure << "\t" << -pStress(1) << "\t" << pPressure - pStress(1) << "\t " << pDispl(1) << "\t";
        }
        if (pid == 300) {
            pPressure = i->give_pressure();
            pPressure = pPressure/1000;
...
void mpm::MpmParticle::write_particle_void_ratio_data_to_file(std::ofstream& outFile) {
      unsigned numOfParticles = particles_.size();
    ...
    outFile << "LOOKUP_TABLE default" << "\n";
    //for (auto i : particles_)
    //    i -> write_void_ratio(outFile);
}

void mpm::MpmParticle::write_particle_preconsolidation_pressure_data_to_file(std::ofstream& outFile) {
      unsigned numOfParticles = particles_.size();
